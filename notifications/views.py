...
def cron_send_reminders(request):
    ...
    # Bulk-fetch sets for today to avoid per-user queries inside the loop
    ...
    # Prefetch active subscriptions grouped by user
    from collections import defaultdict
    active_subs_by_user = defaultdict(list)
    for sub in PushSubscription.objects.filter(
        user_id__in=pref_user_ids, is_active=True
    ):
        active_subs_by_user[sub.user_id].append(sub)
    ...
    for pref in preferences:
        ...
        try:
            ...
            if not force:
                # Check if already reminded today (using pre-fetched set)
                ...
                # Check if already logged today (using pre-fetched set)
            ...
            # Get active subscriptions (using pre-fetched dict)
            subscriptions = active_subs_by_user.get(user.id, [])
